...
const ipToken = import.meta.env.VITE_IPINFO_TOKEN
...
const fetchWeather = async (lat, lon) => {
  try {
    const res = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&lang=en&appid=${weatherApiKey}`
    )
...
const fetchCityImage = async (cityName) => {
  try {
    const res = await fetch(
      `https://api.unsplash.com/search/photos?query=${cityName}&orientation=portrait&client_id=${unsplashApiKey}`
    )
    const data = await res.json()
